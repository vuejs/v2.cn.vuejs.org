...
`v-` 前缀从视觉上提示模板中特殊的 Vue 特性。当你使用 Vue.js 在现成的标签上处理动态行为时这非常好用，但是当你反复高频率的使用指令时，这又会显得笨重。而当你构建单页应用并通过 Vue.js 管理每个模板时，`v-` 前缀不大必要。因此 Vue.js 为两个最常用的指令 `v-bind` 和 `v-on` 提供特别的缩写：
