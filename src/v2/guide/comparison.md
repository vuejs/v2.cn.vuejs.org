...
对于纯组件，渲染输出仅依赖于自身的状态。对于非纯组件，渲染输出依赖于自身状态和父组件状态。
...
随着时间的推移，Knockout 的发展已有所放缓，并且略显有点老旧了。比如，它的组件系统缺少完备的生命周期事件方法，尽管这些在现在是非常常见的。以及相比于 [Vue](components.html#使用 Slots 分发内容) 调用子组件的接口它的方法显得有点笨重。
