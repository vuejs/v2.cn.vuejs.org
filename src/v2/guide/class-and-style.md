...
我们也可以在对象中传入更多属性来动态切换多个 class。此外，`v-bind:class` 指令可以与普通的 class 属性共存。当有如下模板:
...
当在一个定制的组件上使用 `class` 属性的时候，这些类将被添加到根元素上面。这个元素上已经存在的类不会被覆盖。
...
自 2.3.0+ 开始，可以将一个样式属性的属性值设为一个包含多个 (带前缀的) 值的数组，例如：

```html
<div v-bind:style="{ display: ['-webkit-box', '-ms-flexbox', 'flex'] }"></div>
```

这样写只会渲染数组中最后一个浏览器支持的属性值。在本例中，在支持无前缀 flexbox 的浏览器下就只会渲染 `display: flex`。
