...
### 模板中的复杂表达式 <sup data-p="b">强烈推荐</sup>

**组件模板应该只包含简单的表达式，更复杂的表达式则应该重构为计算属性或方法。**

复杂表达式会让你的模板变得不那么声明式。我们应该剥离出_什么_应该出现，而不是_如何_计算那个指。而且计算属性和方法允许代码重用。
...
- __更好的适配需求变更__
  ...
  小的、专注的计算属性让使用信息时的假设变得更少了，所以需求变更时也不需要那么多重构了。
...
在 HTML 中不带空格的特性值时可以没有引号的，这一实践方式常常导致_回避_空格，让特性值的可读性变差。
