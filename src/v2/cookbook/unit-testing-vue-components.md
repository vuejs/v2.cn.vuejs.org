...
在一开始，工厂函数将 `values` 对象合并到了 `data` 并返回了一个新的 `wrapper` 实例。这样，我们就不需要在每个测试中重复 `const wrapper = shallowMount(Foo)`。另一个好处是当你想为更复杂的组件在每个测试中伪造或存根一个方法或computed property时，你只需要声明一次即可。
